...
function c99980060.initial_effect(c)
  ...
  e1:SetTarget(c99980060.stztg)
  e1:SetOperation(c99980060.stzop)
  c:RegisterEffect(e1)
  local e2=e1:Clone()
...
function c99980060.stzfilter(c,tp)
  return c:IsCode(99980080) and not c:IsForbidden()
end
...
function c99980060.stzop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if tc and Duel.MoveToField(tc,tp,tp,LOCATION_SZONE,POS_FACEUP,true)~=0 then
    if Duel.GetFlagEffect(tp,99980060)~=0 then return end
    ...
    Duel.RegisterFlagEffect(tp,99980060,RESET_PHASE+PHASE_END,0,1)
  end
end
--(2) Level 4 Xyz

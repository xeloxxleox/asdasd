...
function c99980160.initial_effect(c)
  ...
  e1:SetTarget(c99980160.stztg)
  e1:SetOperation(c99980160.stzop)
  c:RegisterEffect(e1)
  local e2=e1:Clone()
...
function c99980160.stzfilter(c,tp)
  return c:IsCode(99980180) and not c:IsForbidden()
end
...
function c99980160.stzop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if tc and Duel.MoveToField(tc,tp,tp,LOCATION_SZONE,POS_FACEUP,true)~=0 then
    if Duel.GetFlagEffect(tp,99980160)~=0 then return end
    ...
    Duel.RegisterFlagEffect(tp,99980160,RESET_PHASE+PHASE_END,0,1)
  end
end
--(2) Level 4 Xyz

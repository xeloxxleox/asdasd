--BRS White ★ Rock Shooter
--Scripted by Raivost
function c99960080.initial_effect(c)
  c:EnableReviveLimit()
  --Xyz Summon 
  aux.AddXyzProcedure(c,c99960080.xyzfilter,nil,99,c99960080.ovfilter,aux.Stringid(99960080,0),nil,c99960080.xyzop,false,c99960080.xyzcheck)
  --(1) Attach
  local e1=Effect.CreateEffect(c)
  e1:SetDescription(aux.Stringid(99960080,0))
  ...
  e3:SetCountLimit(1,99960080)
  e3:SetRange(LOCATION_MZONE)
  e3:SetCost(c99960080.movecost)
  e3:SetTarget(c99960080.movetg)
  e3:SetOperation(c99960080.moveop)
  c:RegisterEffect(e3)
  --(4) Destroy
  local e4=Effect.CreateEffect(c)
  e4:SetDescription(aux.Stringid(99960080,4))
  e4:SetCategory(CATEGORY_DESTROY+CATEGORY_RECOVER)
  e4:SetType(EFFECT_TYPE_IGNITION)
  e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
  e4:SetCountLimit(1)
  e4:SetRange(LOCATION_MZONE)
  e4:SetCost(c99960080.descost)
  e4:SetTarget(c99960080.destg)
  e4:SetOperation(c99960080.desop)
  c:RegisterEffect(e4,false,1)
end
--Xyz Summon
function c99960080.xyzfilter(c,xyz,sumtype,tp)
  return c:IsType(TYPE_XYZ,xyz,sumtype,tp) and c:IsSetCard(0x996)
end
function c99960080.xyzcheck(g,tp,xyz)
  local mg=g:Filter(function(c) return not c:IsHasEffect(511001175) end,nil)
  return mg:GetClassCount(Card.GetRank)==1
end
function c99960080.ovfilter(c)
  return c:IsFaceup() and c:IsCode(99960010)
end
function c99960080.xyzop(e,tp,chk,mc)
...
function c99960080.attachtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  ...
  local g=Duel.SelectTarget(tp,c99960080.attachfilter,tp,LOCATION_GRAVE,0,1,1,nil)
...
--(3) Move to Zone
function c99960080.movecost(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return Duel.CheckLPCost(tp,700) end
  Duel.PayLPCost(tp,700)
end
function c99960080.movetg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c99960080.moveop(e,tp,eg,ep,ev,re,r,rp)
  local c=e:GetHandler()
  ...
  and Duel.IsExistingMatchingCard(c99960080.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) 
  and Duel.SelectYesNo(tp,aux.Stringid(99960080,2)) then
    Duel.Hint(HINT_OPSELECTED,1-tp,aux.Stringid(99960080,3))
